...
| 步行路径规划 | `/v5/direction/walking` | 计算地点与接驳站之间的真实步行距离和时间 1 |
...
- `navi` 用于展示分段动作。
- `polyline` 用于在地图上绘制路线；每个 `busline` 乘车段保留线路 ID、名称和交通类型并单独着色，方案内部的换乘步行段绘制为虚线。
- `departure_stop`、`via_stops` 和 `arrival_stop` 用于标注每段乘车线路的上车站、途经站和下车站；相同换乘站在前端合并展示关联线路。
...
1. 使用附近站点的线路信息确定线路种子，并调用 `/v3/bus/linename` 获取完整站序。
2. 依据站点 ID、名称和坐标合并公交、地铁与有轨电车线路中的重复站点。
   线路名匹配前统一移除“地铁”“轨道交通”“轨交”等城市差异前缀。
3. 调用 `/v5/direction/walking` 取得地点与每个接驳站的道路步行耗时，计算公共交通剩余预算。
4. 以候选住所侧站点为起点、公司侧接驳站为终点组成起终点对。
5. 调用 `/v5/direction/transit/integrated` 获取住所到公司的方案。
6. 取符合线路限制的最短 `cost.duration`，并只保留公共交通耗时不超过该接驳站剩余预算的站点。
7. 计算住所到公司方向的地理最远、最接近时间上限和各方位边界站点。
8. 对请求结果按地点、接驳站、线路、时间桶和策略缓存。
...
- `/api/amap/reachability`：从接驳站的真实线路站序生成候选，先计算公司地点与接驳站的步行时间，再以剩余时间核验住所到公司的公共交通路线，并返回预算内站点、最远点和最近未达标候选。
- 高德 JS API 2.0：加载地图、地点标记和站点标记。
- 前端通勤条件：日期、时间和 20～90 分钟住所到公司总预算。
- 前端可达性结果：步行/公共交通预算拆分、展开线路数量、沿线候选数量、有效路线、最远站点、地图分钟标记、当前候选的分线路路线和沿途站点标注；点击候选会切换并高亮该路线。
