...
4. 将接驳站与候选站点组成双向起终点对。
5. 调用 `/v5/direction/transit/integrated` 获取两个方向的方案。
6. 取符合线路限制的最短 `cost.duration`，并只保留公共交通耗时不超过该接驳站剩余预算的站点。
7. 分别计算两个方向的地理最远、最接近时间上限和各方位边界站点。
...
- `/api/amap/reachability`：从接驳站的真实线路站序生成候选，先计算地点与接驳站的步行时间，再以剩余时间分别核验双向公共交通路线，并返回两个方向各自的预算内站点、最远点和最近未达标候选。
- 高德 JS API 2.0：加载地图、地点标记和站点标记。
- 前端通勤条件：日期、时间和 20～90 分钟双向总预算。
- 前端可达性结果：步行/公共交通预算拆分、展开线路数量、沿线候选数量、有效路线、最远站点、地图分钟标记、分线路着色的双向最远路线和沿途站点标注。
